...
for i, entry in enumerate(contacts_feed.entry):
	...
	if len(entry.group_membership_info) > 0:
		...
		if sum([1 for x in entry.group_membership_info
				if '5abdac650bbd1b4c' in x.href]) > 0:
			print('\n',i)

			last_touch_ds = '2000-01-01'
			...
			if now - last_touch_dt > datetime.timedelta(weeks=2):
				...
				os.system("""osascript -e 'display notification "Last touch on {ds}" with title "Reach out to {name}"'""".\
